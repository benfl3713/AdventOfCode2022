...
public static class Part2
{
    public static void Run()
    {
        ...
        var size = GetFolderSize(fileSystem);
        ...
        fileSystem.TraverseNodes(n =>
        {
            ...
            foldersUnder100Thousand.Add(n, GetFolderSize(n));
        });

        const int totalDisk = 70000000;
        int neededSpace = 30000000 - (totalDisk - GetFolderSize(fileSystem));

        var folderToDelete = foldersUnder100Thousand.OrderBy(n => n.Value).First(n => n.Value >= neededSpace);
        Console.WriteLine($"The Folder to delete is: {(folderToDelete.Key.Value as Folder)?.Name} which is {folderToDelete.Value} big");
    }
